...
def test_mascarar_nunca_devolve_a_chave(env):
    assert settings.mascarar("") == ""
    assert settings.mascarar("abc") == "•••"

    chave = "abcdefghijklmnop1234"
    mascarada = settings.mascarar(chave)
...
def test_estado_nao_expoe_o_segredo(env):
    ...
    campos = {c["chave"]: c for c in settings.estado()["campos"]}
    api = campos["CURSEFORGE_API_KEY"]

    assert api["definido"] is True
    assert "supersecreto" not in api["valor"]
    assert api["valor"].endswith("y123")
...
def test_gravar_preserva_comentarios_e_chaves_alheias(env):
    ...
    settings.gravar({"M2CF_WORKERS": "12", "M2CF_HTTP_TIMEOUT": "90"})
...
def test_gravar_ignora_chave_desconhecida(env):
    settings.gravar({"ALGO_INVENTADO": "x", "M2CF_WORKERS": "8"})
...
def test_valor_vazio_comenta_a_linha(env):
    ...
    settings.gravar({"M2CF_WORKERS": ""})
    texto = env.read_text(encoding="utf-8")

    # comentada, não apagada: você vê que a linha existiu
    assert "# M2CF_WORKERS=" in texto
    assert settings.ler().get("M2CF_WORKERS") is None


def test_numero_invalido_nao_grava_nada(env):
    env.write_text("M2CF_WORKERS=6\n", encoding="utf-8")

    resultado = settings.gravar({"M2CF_WORKERS": "muitos", "M2CF_HTTP_TIMEOUT": "30"})

    assert resultado["ok"] is False
    assert "não é um número" in resultado["erros"][0]
...
def test_limites_sao_respeitados(env):
    assert settings.gravar({"M2CF_WORKERS": "999"})["ok"] is False
    assert settings.gravar({"M2CF_WORKERS": "0"})["ok"] is False
    assert settings.gravar({"M2CF_WORKERS": "12"})["ok"] is True
...
def test_limiar_de_similaridade_fica_fora_da_tela(env):
    ...
    assert "M2CF_VERSION_THRESHOLD" not in settings.POR_CHAVE

    # e a tela não o grava nem se alguém mandar
    settings.gravar({"M2CF_VERSION_THRESHOLD": "0.1"})
...
def test_gravar_reflete_no_config_em_memoria(env, monkeypatch):
    ...
    settings.gravar({"M2CF_WORKERS": "10", "M2CF_HTTP_TIMEOUT": "90"})
...
def test_env_inexistente_e_criado(env):
    ...
    settings.gravar({"M2CF_WORKERS": "7"})

    assert env.is_file()
    assert settings.ler()["M2CF_WORKERS"] == "7"


def test_a_chave_traz_o_link_do_console(env):
    """Sem link a tela não teria como mostrar onde gerar a chave."""

    campos = {campo["chave"]: campo for campo in settings.estado()["campos"]}
    ...
    assert [c["chave"] for c in campos.values() if c["link"]] == [
        "CURSEFORGE_API_KEY"
    ]
